...
export default function Dashboard({ setPage }) {
  const { canEdit, setShowLogin } = useAdmin();
      ...
      <div style={{ background: "linear-gradient(135deg,#0f172a 0%,#1e3a5f 60%,#1e4080 100%)", borderRadius: 14, padding: "28px 32px", marginBottom: 24, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", position: "relative", zIndex: 2 }}>
          <button type="button" onClick={() => {
            if (!canEdit("invoices")) { setShowLogin(true); return; }
            if (typeof setPage === "function") setPage("invoices"); else window.location.hash = "invoices";
          }} style={{ background: "#3b82f6", color: "#fff", border: "none", borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 600, cursor: "pointer", position: "relative", zIndex: 2 }}>+ New Invoice / Qtn</button>
