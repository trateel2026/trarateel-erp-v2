      ...
      <div style={{ background:"#fff",borderRadius:14,padding:24,border:"1px solid #e2e8f0",marginTop:8 }}>
        ...
        {isAdmin && (
          <div style={{ marginTop:16,padding:16,background:"#fffbeb",borderRadius:10,border:"1px solid #fde68a" }}>
            ...
            <button onClick={async()=>{
              const {supabase:sb}=await import("../lib/supabase");
              const { data, error } = await sb.from("ledger").select("payment_mode, type, amount");
            ...
            <div style={{ display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" }}>
              ...
              <button onClick={async()=>{
                ...
                const {supabase:sb}=await import("../lib/supabase");
                const { data: rows, error: cErr } = await sb.from("ledger").select("id").eq("payment_mode",fromMode);
                ...
                const { error: uErr } = await sb.from("ledger").update({ payment_mode: target }).eq("payment_mode",fromMode);
            ...
            <div style={{ marginTop:12,paddingTop:12,borderTop:"1px solid #fde68a" }}>
              <button onClick={async()=>{
                ...
                const {supabase:sb}=await import("../lib/supabase");
                const { data: rows } = await sb.from("ledger").select("id").neq("payment_mode",target);
                const n=(rows||[]).length;
                ...
                const { error } = await sb.from("ledger").update({ payment_mode: target }).neq("payment_mode",target);
